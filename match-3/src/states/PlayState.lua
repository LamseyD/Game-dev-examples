...
function PlayState:enter(params)
    ...
    self.board = params.board or Board({
                x = VIRTUAL_WIDTH - 272, 
                y = 16, 
                level = self.level
    })
    ...
    self.scoreGoal = self.level * 1.75 * 5000
end
    ...
    if self.canInput then
        ...
        if love.keyboard.wasPressed('enter') or love.keyboard.wasPressed('return') then
            ...
            elseif math.abs(self.highlightedTile.gridX - x) + math.abs(self.highlightedTile.gridY - y) > 1 then
                ...
                -- local temp_tile = self.board.tiles[y][x]
                -- self.board:swap_tiles(self.highlightedTile, self.board.tiles[y][x])                
                ...
                if not self:calculateMatches() then
                    --self.board:swap_tiles(self.highlightedTile, temp_tile)
                end
                self.highlightedTile = nil
                self.canInput = true
            end
...
function PlayState:calculateMatches()
    ...
    if matches then
        ...
        for k, match in pairs(matches) do
            ...
            for j, tile in pairs(match) do 
                extra_points = extra_points + (tile.variety - 1) * 100
            end
            self.score = self.score + #match * 50 + extra_points
        end
    ...
    --     self.board:swap_tiles(self.highlightedTile, self.board.tiles[self.boardHighlightY+1][self.boardHighlightX+1])
    -- -- end
    --     --     self.canInput = true
    --     self.highlightedTile = nil
    end
...
function PlayState:render()
    ...
    if self.rectHighlighted then
        love.graphics.setColor(0.851, 0.341, 0.388, 1)
    else
        love.graphics.setColor(0.675, 0.196, 0.196, 1)
    end
    ...
    -- GUI text
    love.graphics.setColor(0.220, 0.220, 0.220, 0.90)
    love.graphics.rectangle('fill', 16, 16, 186, 116, 4)

    love.graphics.setColor(0.388, 0.608, 1, 1)
